...
namespace ProyectoPOA.Repositories
{
    public class PartidasRepository:Repository<Partida>
    {
        ...
        public Boolean Eliminar(Int16 Clave)
        {
            Partida p = GetPartidaXId(Clave);
...
public Boolean Validar(Partida p, out List<String> errores)
        {
            ...
            else
            {
                ...
                {
                    Partida part = Context.Partida.FirstOrDefault(x => x.Concepto.Trim().ToUpper() == p.Concepto.Trim().ToUpper() && x.Eliminado == false && x.Clave != p.Clave);
                    if (part != null)
                    {
        ...
        public Partida GetPartidaXId(Int16 Clave)
        {
            return Context.Partida.FirstOrDefault(x => x.Eliminado == false && x.Clave == Clave);
        }
