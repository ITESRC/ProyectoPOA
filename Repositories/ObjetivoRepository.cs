...
namespace ProyectoPOA.Repositories
{
    public class ObjetivoRepository : Repository<Objetivo>
    {
        ...
        {
            var Vigente = Context.Objetivo.Where(x => x.Eliminado == true);
            return Vigente;
        }
        ...
        public List<Estrategia> GetEstrategiasByObjetivo(int IdObjetivo)
        {
        ...
        //}
        public List<string> Validar(Objetivo VO)
        {
            List<string> errores = new List<string>();
            var repetido = Context.Objetivo.Any(x => x.Nombre == VO.Nombre && x.Eliminado == false);
